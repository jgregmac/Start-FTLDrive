...
function Get-PrivAccountUsage {
    ...
    #Retrieves all scheduled tasks and their 'runas' accounts from the server specified in '$computername'
    [CmdletBinding()]
        param(
            [Parameter(
            Mandatory=$true,
        ...
        [xml]$xmlTasks = schtasks.exe /query /s $ComputerName /XML ONE
